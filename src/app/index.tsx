...
function App() {
	...
	const checkOtaUpdate = async () => {
		try {
			...
			if (result === codePush.SyncStatus.UPDATE_INSTALLED) {
				Alert.alert('Updated', 'The app will reload with the latest update.');
			} else if (result === codePush.SyncStatus.UP_TO_DATE) {
				setStatus('No OTA update available');
				Alert.alert('No update', 'Your app is already up to date.');
			} else {
				setStatus('Update was not installed');
				Alert.alert('Update not installed', `CodePush status: ${result}`);
			}
		} catch (error) {
			console.error(error);
